...
if($method =~ /^(1)$/){
    # Encrypt
    $string = "Type Something Here!"; #### Type something here to encrypt!
    print my_encrypt($string);
}
if($method =~ /^(2)$/){
    # Decrypt
    $string = read_file('raymtm.txt'); #### After Encrypting a string, it gives you a file name, input it here
    $key_value = "W-p=vFoW"; #### After Encrypting a string, it gives you a "Key" input it here
...
$semi_encrypted_string = encode_base64(encode_base64($new_lap));
    ...
    # This should be return: {
        print color("red"), "\n================================\n", color("reset");
        print color("blue"), "Encrypted String located in file:\n", color("reset");
